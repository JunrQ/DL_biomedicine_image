...
MAIN_LOG_LOC = "./multiple_deepmiml_30/"
...
def run_for_dataset(config, train_set, test_set, log_dir, pipe):
    ...
    validation_data = dm.get_validation_stream()
    test_data = dm.get_test_stream(batch_size=64)
    model = DeepMiml(config, 10, is_finetuning=True, label_weights=dm.get_imbalance_ratio().train.values)

    tf.reset_default_graph()
    train_config = TrainConfig(model=model, dataflow=train_data,
                               callbacks=[
                                   ScheduledHyperParamSetter(
                                       'learning_rate', [(0, 1e-5), (40, 1e-5)]),
